...
<sub><sup>UML Model Class Diagram</sup></sub>
...
![Sample Collection Sequence Diagram](diagrams/sample-collection-seq.png)
<sub><sup>Sample Collection Sequence Diagram</sup></sub>
...
To access the REST API open the URL > [http://localhost:8080/](http://localhost:8080/)

## REST API Generated by Swagger

Command line for API generation:
```batch
java -jar swagger-codegen-cli-2.4.5.jar generate --lang spring -i swagger.yaml -c config.json -o testmanager-gen-api
```